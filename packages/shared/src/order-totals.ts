import type { DiscountScope, DiscountType } from './enums.js';
import { round2 } from './money.js';

/**
 * Pure order-money math (spec §2.5/§2.6), shared by the API (authoritative
 * computation + split allocation) and the web POS (provisional cart totals and
 * split-bill part totals). Keeping ONE implementation guarantees the client
 * sends payment amounts that match the server's totals to the cent — otherwise
 * the tender-equals-total check rejects the payment.
 */
...
export function computeOrderTotals(
  ...
  const subtotal = round2(active.reduce((sum, l) => sum + lineTotalOf(l), 0));
...
export function allocateProportional(amount: number, weights: number[]): number[] {
  ...
  const remainder = cents - distributed;
