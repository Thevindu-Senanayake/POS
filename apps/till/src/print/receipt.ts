...
/**
 * The rendering surface the layout code draws onto. In the till the concrete
 * implementation is {@link HtmlSink} (renders to an HTML document that Chromium
 * prints via the installed Windows driver); the method set mirrors the subset of
 * `node-thermal-printer` the original agent used, so the layout below stays
 * device-agnostic and identical to the ESC/POS output venues are used to.
 */
export interface ReceiptSink {
  ...
  drawLine(): void;
  newLine(): void;
  /** Print the venue logo centered (bills only). Async to match the thermal driver's contract. */
...
export async function renderBill(sink: ReceiptSink, p: BillPayload): Promise<void> {
  ...
  // --- Logo (owner toggle) — printed above the header. ---
