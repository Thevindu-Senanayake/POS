/**
 * Print payload + job shapes, kept local to the till so the Electron shell has
 * no workspace-type coupling (it gets packaged into a standalone .exe in phase
 * 3). These mirror `PrintJobAgentDTO` and the `buildKotPayload`/`buildBillPayload`
 * shapes the API emits; the guards defend the print host against anything
 * unexpected on the queue.
 */

export type PrintJobType = 'kot' | 'bill';
export type Station = 'kitchen' | 'bar';
/** `kitchen`/`bar` route KOTs; `receipt` takes station-less bill jobs. */
export type PrinterRole = 'kitchen' | 'bar' | 'receipt';

/** One claimed job as the API's `/printing/agent/claim` returns it. */
