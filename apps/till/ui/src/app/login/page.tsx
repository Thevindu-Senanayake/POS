...
const ADMIN_BLOCKED_MSG =
  'Admin accounts cannot log into the POS Till App. Please use the Web Portal.';
...
export default function LoginPage() {
  ...
  // Already signed in → the till is operational-only, so bounce admins back out.
  useEffect(() => {
    if (hydrated && token) {
      const currentUser = useAuthStore.getState().user;
      if (currentUser && currentUser.role === 'admin') {
        clear();
        setFormError(ADMIN_BLOCKED_MSG);
        return;
      }
      router.replace('/');
    }
      ...
      <div className="card w-full max-w-sm overflow-hidden">
        ...
        <div className="p-7">
          <div className="mb-6 flex flex-col items-center text-center">
            ...
            <p className="mt-1 text-sm text-slate-500">Sign in to POS Till Terminal</p>
          </div>
