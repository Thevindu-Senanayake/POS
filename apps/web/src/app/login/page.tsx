...
import { api, ApiError, Button, Spinner, useAuthStore } from '@pos/client-core';
...
const ADMIN_ONLY_MSG = 'Only Admins can log into the Web Management Portal.';
...
export default function LoginPage() {
  ...
  // Already signed in → this portal is admin-only, so bounce non-admins out.
  useEffect(() => {
    if (hydrated && token) {
      const currentUser = useAuthStore.getState().user;
      if (currentUser && currentUser.role !== 'admin') {
        clear();
        setFormError(ADMIN_ONLY_MSG);
        return;
      }
      router.replace('/');
    }
      ...
      <div className="card w-full max-w-sm overflow-hidden">
        ...
        <div className="p-7">
          <div className="mb-6 flex flex-col items-center text-center">
            ...
            <h1 className="text-2xl font-extrabold tracking-tight text-gradient">Grand Admin</h1>
            <p className="mt-1 text-sm text-slate-500">Sign in to Web Management Portal</p>
          </div>
